...
OAUTH_TOKEN = '1123524671956234240-upXtRVU5S2nL9KMLAjcEAJWf4LwhsX'
OAUTH_TOKEN_SECRET = '9bU6oCuIiQbJBKfyDtM8E8zAVbpsVv8v0jkFmeN18ES6i'
...
def get_tweet_list(twapi, idlist):
    ...
    tweets = twapi.statuses_lookup(
        id_=idlist, include_entities=False, trim_user=True)
    if len(idlist) != len(tweets):
        Logger.warn('get_tweet_list: unexpected response size %d, expected %d', len(
            tweets), len(idlist))
    for tweet in tweets:
    ...
    with open(idfilepath, 'rb') as idfile:
        for line in idfile:
            ...
            if len(tweet_ids) == batch_size:
                Logger.debug(
                    'get_tweets_bulk: fetching batch of size %d', batch_size)
                get_tweet_list(twapi, tweet_ids)
                tweet_ids = list()
    # process remainder
    if len(tweet_ids) > 0:
        Logger.debug(
            'get_tweets_bulk: fetching last batch of size %d', len(tweet_ids))
        get_tweet_list(twapi, tweet_ids)
...
'''
    ...
    '''
    if bulk:
        get_tweets_bulk(api, 'idCounts.txt')
    else:
        '''
    get_tweets_single(api, 'idCounts.txt')
